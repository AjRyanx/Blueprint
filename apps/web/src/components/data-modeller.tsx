...
export function DataModeller({ projectId }: Props) {
  ...
  const saveMutation = useMutation({
    mutationFn: (d: DataModelData) => {
      // Filter and deep clean nulls/undefined to pass Zod validation
      const cleaned = {
        entities: (d.entities || [])
        ...
        relationships: (d.relationships || [])
          .filter((r) => (r.name || '').trim() !== '' && (r.source || '').trim() !== '' && (r.target || '').trim() !== '')
          .map((r) => ({
            name: r.name,
        ...
        indexes: (d.indexes || [])
          .filter((i) => (i.name || '').trim() !== '' && (i.entity || '').trim() !== '')
          .map((i) => ({
            name: i.name,
            entity: i.entity,
            columns: i.columns || i.fields || [],
            unique: !!i.unique,
          })),
        notes: d.notes || '',
      };
      return saveDataModel(projectId, token, cleaned as DataModelData);
    },
    onSuccess: () => {
  ...
  const generateMutation = useMutation({
    ...
    onSuccess: (generatedData) => {
      ...
      // Ensure generated data is also "safe" and normalized
      const normalized = {
        entities: (generatedData.entities || []).map((ent) => ({
        ...
        relationships: (generatedData.relationships || []).map((rel) => ({
        ...
        indexes: (generatedData.indexes || []).map((idx) => ({
          name: idx.name || 'index',
          entity: idx.entity || '',
          columns: idx.columns || idx.fields || [],
